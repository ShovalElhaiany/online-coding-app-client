...
const CodeBlockPage = ({ codeBlock }) => {
  ...
  useEffect(() => {
    const newSocket = io("online-coding-app-server:5000");
  ...
  useEffect(() => {
    const checkIfMentor = async () => {
      const response = await fetch("http://online-coding-app-server:5000/user_info", {
        headers: {
      ...
      if (data.isMentor) {
        alert("Hi Tom :)");
        await fetch("http://online-coding-app-server:5000/user_info", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
